...
function displayAPILimits() {
    ...
    html += `
        <div style="margin-top:0.5rem;"><strong>🔥 Firebase (Daily Quota - Free Tier):</strong></div>
        <div style="font-size:0.8rem;color:#555;">
            Reads: ~${productCount} / ${firebaseReads} 
        ...
        <div style="font-size:0.8rem;color:#555;">
            ...
            <span style="color:${writePercent > 50 ? '#dc3545' : '#28a745'};">(${writePercent}%)</span>
        </div>
        <div style="width:100%;background:#e9ecef;border-radius:8px;height:4px;margin:2px 0;">
            <div style="width:${writePercent}%;background:${writePercent > 50 ? '#dc3545' : '#28a745'};border-radius:8px;height:4px;"></div>
        </div>
        <div style="margin-top:0.4rem;font-size:0.7rem;color:#888;">
            ⚡ Product count: ${productCount} | Pages: ${Math.ceil(productCount / 20) || 1}
        </div>
    `;
...
function getRapidAPILimitFromStorage() {
    try {
        const saved = localStorage.getItem("rapidapi_limit");
        if (saved) return JSON.parse(saved);
    } catch (e) {}
    return null;
}

function saveRapidAPILimit(limit) {
    if (limit) {
        localStorage.setItem("rapidapi_limit", JSON.stringify(limit));
    }
}
