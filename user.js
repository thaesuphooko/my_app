...
// ========================================================================
// 2. PRODUCT DETAIL WITH IMAGES & REVIEWS
// ========================================================================
...
function openProductDetail(productId) {
  ...
  // Show additional images if ASIN exists
  if (p.asin) {
  ...
  orderTimerInterval = setInterval(() => {
    ...
    if (timeLeft <= 0) {
      ...
      if (currentOrder) {
        ...
        logUserAction(`⛔ Order cancelled (timeout)`, `#${currentOrder.id}`);
      }
    }
  }, 1000);
}
...
document.getElementById("checkoutBtn")?.addEventListener("click", () => {
  ...
  document.getElementById("labelName").innerText = config.checkoutLabels.name || "အမည်";
  document.getElementById("labelPhone").innerText = config.checkoutLabels.phone || "ဖုန်းနံပါတ်";
  document.getElementById("labelAddress").innerText = config.checkoutLabels.address || "လိပ်စာ";
  document.getElementById("submitCheckout").innerText = config.checkoutLabels.submit || "ဆက်လုပ်ရန်";
...
document.getElementById("checkoutForm")?.addEventListener("submit", (e) => {
  ...
  logUserAction(`📦 Checkout`, `Order #${currentOrder.id}, Total: ${currentOrder.total}`);

  const orderSummary = `📦 Order #${currentOrder.id}\n👤 ${name}\n📞 ${phone}\n📍 ${address}\n🛒 ${currentOrder.items.map(i => `${i.name} x ${i.quantity}`).join(', ')}\n💰 ${currentOrder.total.toLocaleString()} MMK`;
...
document.getElementById("uploadScreenshotBtn")?.addEventListener("click", () => {
  ...
  reader.onload = function(e) {
    if (currentOrder) {
      ...
      logUserAction(`✅ Order confirmed with screenshot`, `#${currentOrder.id}`);
    }
    ...
    setTimeout(() => {
      ...
      addChatMessage("bot", `🎉 ဟုတ်ကဲ့ခင်ဗျာ၊ သင့်အော်ဒါ #${currentOrder.id} အတည်ပြုပြီးပါပြီ။ ကျွန်ုပ်တို့ ပို့ဆောင်ပေးပါမည်။ ကျေးဇူးပါ။`);
    }, 1500);
  };
  reader.readAsDataURL(fileInput.files[0]);
});
...
document.getElementById("cancelOrderBtn")?.addEventListener("click", () => {
  if (confirm("Cancel order?")) {
    if (currentOrder) {
      currentOrder.status = "cancelled";
      updateOrderStatus(currentOrder.id, "cancelled");
      logUserAction(`⛔ Order cancelled by user`, `#${currentOrder.id}`);
    }
